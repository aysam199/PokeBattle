...
const LeftPlayer = ({ pokemon, setPokemon, setMove, turn, player}) => {
  ...
  return (
    <section className="player">
      <h1>{player}</h1>
      <h3>{(turn ===1) ? <div>Your turn</div> : <div></div>}</h3>
      ...
      <div id="leftPlayer">
        ...
        <ul className="moves">
          ...
          <li>
            <img
              onClick={() => (turn === 1 ? setMove({ power: move.power, accuracy: move.accuracy }) : '')}
              alt="pokgeball"
              className="pokeball"
              src="./pokeball.png"
            ></img>
      ...
      <img id="characterTwo" src={pokemon.sprites.front_default}></img>
